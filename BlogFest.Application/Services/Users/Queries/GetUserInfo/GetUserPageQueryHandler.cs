...
{
    public class GetUserPageQueryHandler : IRequestHandler<GetUserPageQuery, UserPageDTO>
    {
        ...
        public async Task<UserPageDTO> Handle(GetUserPageQuery request, CancellationToken cancellationToken)
        {
            ...
            var sql = $@"
                        SELECT COUNT(c.Id) FROM (
                            ...
                            when @Id is not null and cu.Id = @Id then 1
                            else 0
                            end = 1
                            AND CASE
                        ...
                        SELECT u.Id, u.Name, UserFileId.Path, u.Bio, u.Active as IsActive, p.Id, p.Slug, p.Title, p.UserId, p.Status, p.DateCreated,  ISNULL(fp.Path, dfp.Path) as ImagePath, p.Text  FROM {DbConstants.UserTable} u
                        OUTER APPLY (
                        ...
                        WHERE case
                            ...
                            when @Id is not null and u.Id = @Id then 1
                            else 0
                            end = 1;
                        ";
            ...
            if (model != null && request.CurrentUserId == request.Id) model.IsUserAccount = true;
